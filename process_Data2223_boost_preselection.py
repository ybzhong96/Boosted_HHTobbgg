...
DATA_2223_DIR = Path("/home/zhong202/depot/users/zhong202/HHtoBBGG/HDNA_sample/v3_parquet_processing/Data")
...
def add_fatjet_base_variables(df):
    for i in range(1, 5):
        ...
        if f"fatjet{i}_mass" in df.columns and f"fatjet{i}_particleNet_massCorr" in df.columns:
            df[f"fatjet{i}_regmass"] = df[f"fatjet{i}_mass"] * df[f"fatjet{i}_particleNet_massCorr"]


def choose_xbb_columns(df, year):
    xbb_cols = [f"fatjet{i}_particleNet_XbbVsQCD" for i in range(1, 5)]
    if all(col in df.columns for col in xbb_cols):
        return xbb_cols
    missing = [col for col in xbb_cols if col not in df.columns]
    raise KeyError(f"Cannot find complete particleNet Xbb score columns for {year}. Missing: {missing}")
...
def add_btagging(df, year):
    ...
    df["fatjet_selected_Xbb_wp2"] = ((xbb >= 0.83) & (xbb < 0.89)).astype(int)
    df["fatjet_selected_Xbb_wp3"] = ((xbb >= 0.89) & (xbb < 0.925)).astype(int)
    df["fatjet_selected_Xbb_wp4"] = ((xbb >= 0.925) & (xbb < 0.96)).astype(int)
    df["fatjet_selected_Xbb_wp5"] = (xbb >= 0.96).astype(int)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Apply Boost_skim_2223-style particleNet fatjet preselection to 2022/2023 data.")
    parser.add_argument("--input-dir", type=Path, default=DATA_2223_DIR)
...
def main():
    args = parse_args()
    files = sorted((args.input_dir / "2022").glob("*_NOTAG_merged.parquet"))
    files.extend(sorted((args.input_dir / "2023").glob("*_NOTAG_merged.parquet")))
